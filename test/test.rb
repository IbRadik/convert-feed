...
class ReaderTest < MiniTest::Unit::TestCase
  def test_read_file
    test_reader = Reader.new("../test/fixtures/news_atom_test", "rss", "-r", '')
    data = test_reader.read_data("../test/fixtures/news_atom_test")
    rss_feed = File.open('../test/fixtures/news_atom_test') { |f| Nokogiri::XML(f) }
  ...
  def test_storage_data
    ...
    test_reader = Reader.new("../test/fixtures/news_atom_test", "rss", "-r", '')
    data = test_reader.storage_data(File.open('../test/fixtures/news_atom_test') { |f| Nokogiri::XML(f) })
